...
public class CreateRoleHandler(IUserRoleRepo userRoleRepo,AuthenticatedUserInfo authenticatedUserInfo):ICommandHandler<CreateRoleCommand, CreateRoleResponse>
{
    public async Task<CreateRoleResponse> Handle(CreateRoleCommand request, CancellationToken cancellationToken)
    {
        ...
        var createdUserRole = await userRoleRepo.CreateAsync(userRole, authenticatedUserInfo.Id,cancellationToken);
